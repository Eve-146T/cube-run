...
class DuckBarReviewTest {
    ...
    @Test fun captureCandidates() {
        ...
        val world = arguments.getString("duckWorld")?.toInt()
        val styles = ObstacleCueStyle.entries.filter { arguments.getString("duckOnly") != "true" || it.duckOnly }
        val previousWorld = Settings.testWorld
        if (world != null) Settings.testWorld = world
        try {
                ...
                try {
                    for (scene in listOf("duck", "jump", "hills", "moving")) {
                        ...
                        for (style in styles) {
                            ...
                            try {
                                val prefix = if (world == null) "" else "$world-"
                                File(dir, "$prefix${style.ordinal}-$scene.png").outputStream().use {
        ...
        } finally { Settings.testWorld = previousWorld }
    }
}
