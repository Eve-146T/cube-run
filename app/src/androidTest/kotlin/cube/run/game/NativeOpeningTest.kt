...
class NativeOpeningTest {
    @Test fun loadingDoesNotRestartTheClockAndBackgroundTimeDoesNotAdvanceIt() {
        ...
        assertEquals("Adopting system rotation must not restart the launch deadline", before, clock.seconds(), .02f)
        assertEquals(.3f, clock.motionSeconds(), .03f)
        assertEquals(0f, clock.sceneSeconds(), 0f)
        assertEquals(0f, clock.skinAmount(), 0f)
        clock.releaseSystem()
        assertEquals(0f, clock.sceneSeconds(), .001f)
        clock.pause()
    ...
    @Test fun anExpiredSystemAnimationTransfersItsFinalPoseAndContinuesAfterRelease() {
        val clock = OpeningClock()
        clock.start(); clock.pause()
        clock.adoptSystemStart(System.currentTimeMillis()-5000, 1000)
        assertEquals("A completed one-second vector must not transfer a later spin", 1f, clock.motionSeconds(), .001f)
        assertEquals(0f, clock.sceneSeconds(), 0f)
        assertEquals(0f, clock.skinAmount(), 0f)
        clock.releaseSystem()
        assertEquals(1f, clock.motionSeconds(), .001f)
        assertEquals(0f, clock.sceneSeconds(), .001f)
        clock.resume()
        SystemClock.sleep(180)
        assertTrue(clock.motionSeconds() > 1.15f)
        assertTrue(clock.sceneSeconds() in .1f..OpeningPose.DURATION)
        clock.pause()
        val scene = clock.sceneSeconds()
        SystemClock.sleep(50)
        assertEquals(scene, clock.sceneSeconds(), .001f)
        clock.finish()
        assertEquals(OpeningPose.DURATION, clock.sceneSeconds(), 0f)
        assertEquals(1f, clock.skinAmount(), 0f)
    }
