...
class AbilityDisplay(private val activity: Activity, private val kit: UiKit, val style: Int = 0) {
    ...
    private fun description(ability: Ability, dark: Boolean = false) =
        if (dark) kit.stageText(kit.ctx.gameText(ability.detail), 15f, stroke = 1.8f, weight = 500, gravity = Gravity.CENTER)
        else kit.text(kit.ctx.gameText(ability.detail), 15f, Theme.INK, 500, Gravity.START)
        ...
        for ((i, ability) in values.withIndex()) {
            ...
            section.addView(row().apply {
                if (icons) addView(ImageView(activity).apply { setImageDrawable(icon(ability)) },
                    LinearLayout.LayoutParams(dp(26f), dp(26f)).apply { marginEnd = dp(8f) })
                addView(kit.text(kit.ctx.gameText(ability.title), 17f, Theme.INK, 700, Gravity.START),
                    LinearLayout.LayoutParams(0, -2, 1f))
            })
            if (ability == Ability.LOTTERY) {
                val marker = "__COIN__"
                val parts = activity.getString(R.string.lottery_explanation, marker).split(marker, limit = 2)
                val label = android.text.SpannableStringBuilder(parts[0]).append(kit.coins("", 14f)).append(parts.getOrElse(1) { "" })
                ...
                odds(CoinIcon(), if (cube.run.data.Settings.devMode) "2%" else activity.getString(R.string.lottery_one_in), activity.getString(R.string.lottery_per_coin))
                odds(BoxIcon(), "1.3%", activity.getString(R.string.lottery_per_box))
