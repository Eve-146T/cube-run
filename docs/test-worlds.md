...
In the debug build, open the section grid from the main menu. The seven **Test Worlds** cards appear above the individual sections. Choose a card, then tap to start. Each course repeats; restarting begins the course again. **Play normally** in the grid clears the selection. Choosing an individual section also clears it. Release builds hide both the Dev button and section grid.
...
For automation, debug launch intents accept `--ei scenario 0` through `6`; `--ei scenario -1` clears the preset. Selection lasts for the app process, like the existing individual-section selector.
